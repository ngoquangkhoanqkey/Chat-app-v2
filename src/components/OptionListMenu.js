...
function OptionListMenu(props) {
  ...
  const appConfig = JSON.parse(localStorage.getItem("quangkhoa_config")) ?? {
    appThem: "light",
  };

  const setAppConfig = (key,value)=>{
    appConfig[key] = value;
    localStorage.setItem('quangkhoa_config', JSON.stringify(appConfig))
  }

  const [them, setTheme] = useState(appConfig.appThem);
  ...
  const handleChangeTheme = () => {
    if(them ==='light'){
        setTheme('dark');
        setAppConfig('appThem','dark');
    }else{
        setTheme('light');
        setAppConfig('appThem','light');
    }
  };
  ...
  return (
    <div
      className="option-list-wrapper for-topnav"
      style={{ height: optionListHeight }}
    >
      <div
        className={`option-list-wrapper__pages-wrapper page-${idOfPageToBeDisplayed}`}
      >
        {/* Page 1: */}
        ...
        <div className="option-list-wrapper__page" ref={optionListPageRef_2}>
          {/* Menu 1: */}
          <div
            className={`option-list-wrapper__menu${
              idOfMenuToBeDisplayed === 1 ? " is-displayed" : ""
            }`}
          >
            <div className="navigation">
              <div className="navigation__left-section">
                <div
                  className="navigation_button go-back-btn"
                  onClick={() => goToMainMenu()}
                >
                  <FontAwesomeIcon
                    className="navigation_button-icon"
                    icon={faChevronLeft}
                  />
                </div>
          ...
          <div
            className={`option-list-wrapper__menu${
              idOfMenuToBeDisplayed === 2 ? " is-displayed" : ""
            }`}
          >
            <div className="navigation">
              <div className="navigation__left-section">
          ...
          <div
            className={`option-list-wrapper__menu${
              idOfMenuToBeDisplayed === 3 ? " is-displayed" : ""
            }`}
          >
            <div className="navigation">
              <div className="navigation__left-section">
            ...
            <div className="option-list">
              <div className="option-list__item is-disabled">
                <div
                  ...
                  className="option-list__option-title"
                >
                  <div className="option-list__icon-wrapper">
                    <FontAwesomeIcon
                      className="option-list__icon"
                  ...
                  <span>Chế độ tối: {them === 'light' ? "Đăng tắt" : "Đang bật"}</span>
                </div>
              </div>
            </div>
          </div>
